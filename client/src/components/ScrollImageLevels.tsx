...
// ~1 viewport of scroll per level so each one holds long enough to read.
const SCROLL_TRACK_VH_PER_LEVEL = 110;
// Title overlay fades over the first slice of scroll (within the entrance level).
const INTRO_FADE = 0.035;

export interface HeroLevel {
  /** Empty for the entrance level — hides its stepper dot and caption card. */
  no: string;
  tone: string;
  title: string;
  units: string;
  detail: string;
  /** Still image for this level. */
  imageSrc: string;
}

interface ScrollImageLevelsProps {
  eyebrow?: string;
  title: ReactNode;
  scrollHint?: string;
  levels: HeroLevel[];
...
/**
 * Pinned hero driven entirely by still images — no video, no autoplay. Scroll
 * position selects the active level; its image crossfades in while the rest
 * fade out, with the caption and stepper following along.
 */
export function ScrollImageLevels({ eyebrow, title, scrollHint, levels }: ScrollImageLevelsProps) {
  ...
  return (
    <section
      ...
      style={{ height: `${SCROLL_TRACK_VH_PER_LEVEL * levels.length}vh` }}
    >
      ...
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        {/* Vertical floor stepper — skips the entrance level, hidden on small screens */}
        ...
        {/* Per-floor caption — hidden for the entrance level */}
