      ...
      <div className="container mx-auto p-6">
        ...
        <Dialog open={showRewardDialog} onOpenChange={setShowRewardDialog}>
          ...
          <TabsContent value="payment-verifications" className="space-y-6">
            <Card className="bg-white/10 backdrop-blur border-gray-300/30">
              <CardHeader>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <Label className="text-gray-300">Search by User</Label>
                      <Input
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="border-gray-600">
                          <TableHead className="text-gray-300">User</TableHead>
                          <TableHead className="text-gray-300">Amount</TableHead>
                          <TableHead className="text-gray-300">Credits Requested</TableHead>
                      ...
                      <TableBody>
                        {paymentVerifications
                          .filter((verification: any) => {
                            const searchMatch = !userSearch || 
                              verification.user?.firstName?.toLowerCase().includes(userSearch.toLowerCase()) ||
                              verification.user?.email?.toLowerCase().includes(userSearch.toLowerCase());
                            const statusMatch = statusFilter === "all" || verification.status === statusFilter;
                            return searchMatch && statusMatch;
                          })
                          .map((verification: any) => (
                            <TableRow key={verification.id} className="border-gray-600">
                              <TableCell className="text-white">
                                <div>
                              ...
                              <TableCell className="text-white">
                                {formatCurrency(verification.creditsRequested)}
                              </TableCell>
                              <TableCell className="text-white">
                                    ...
                                    <DialogContent className="max-w-md">
                                      ...
                                      <div className="space-y-4">
                                        ...
                                        <div className="text-sm text-gray-600">
                                          <p><strong>Amount:</strong> {formatCurrency(verification.amount)}</p>
                                          <p><strong>Credits:</strong> {formatCurrency(verification.creditsRequested)}</p>
                                          <p><strong>Submitted:</strong> {formatDate(verification.createdAt)}</p>
                                        </div>
                                      </div>
                                    </DialogContent>
                                  </Dialog>
                                    ...
                                    <Button
                                      ...
                                      onClick={() => {
                                        ...
                                        const approveMutation = useMutation({
                                          mutationFn: async () => {
                                            await apiRequest("POST", `/api/admin/payment-verifications/${verification.id}/approve`);
                                          },
                                          onSuccess: () => {
                                            toast({
                                              title: "Success",
                                    ...
                                    <Button
                                      ...
                                      onClick={() => {
                                        ...
                                        const rejectMutation = useMutation({
                                          mutationFn: async () => {
                                            await apiRequest("POST", `/api/admin/payment-verifications/${verification.id}/reject`);
                                          },
                                          onSuccess: () => {
                                            toast({
                                              title: "Success",
                  ...
                  {paymentVerifications.length === 0 && (
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-400">No payment verifications found</p>
                    </div>
                  )}
                </div>
              </CardContent>
