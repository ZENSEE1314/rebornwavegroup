  ...
  useEffect(() => {
    ...
    async function createScene() {
      ...
      if (activeFloor.scene === "live") {
        ...
        addWall(-2.5, -1.98, 0.08, 1.05);
        addWall(2.5, -1.98, 0.08, 1.05);
        addBox([2.75, 0.42, 1.0], [0, 0.22, -2.02], violetMat);
        addBox([2.45, 1.25, 0.08], [0, 0.92, -2.54], mat("#111827", 0.38, 0.16));
        for (let i = 0; i < 4; i += 1) {
          addCyl(0.07, 0.95, [-1.05 + i * 0.7, 0.62, -1.92], mat("#e5e7eb"), [0.1, 0, 0]);
          addCyl(0.16, 0.18, [-1.05 + i * 0.7, 0.38, -1.78], i % 2 ? cyanMat : pinkMat);
        }
        for (let row = 0; row < 4; row += 1) {
          for (let col = 0; col < 7; col += 1) {
            const x = -2.1 + col * 0.7;
            const z = -0.72 + row * 0.54;
            addCyl(0.12, 0.32, [x, 0.2, z], row % 2 ? goldMat : mat("#f8fafc"));
            addCyl(0.09, 0.16, [x, 0.46, z - 0.03], col % 3 ? cyanMat : pinkMat);
          }
        }
        addBox([1.35, 0.12, 1.35], [1.9, 0.1, 0.18], goldMat, [0, 0.78, 0]);
        addBox([2.5, 0.22, 0.46], [1.15, 0.18, 1.95], greenMat);
      }
...
function RenovatedFloorPlanShowcase({
  ...
  const activeFloor = floors[activeIndex] || floors[0];
  const planKey = (activeFloor?.baseLevel || activeFloor?.level || "1F").toLowerCase();
  ...
  const floorPlanRooms: Record<string, Array<{ label: string; className: string }>> = {
    "1f": [
      ...
      { label: "Kids Fun House", className: "pink l1-front-left" },
    ...
    "3f": [
      { label: "VIP KTV Room A", className: "gold l3-vip-a" },
      { label: "VIP KTV Room B", className: "violet l3-vip-b" },
    ...
    "4f": [
      { label: "Pet Cafe Hall", className: "green l4-cafe" },
      { label: "Pet Play Zone", className: "cyan l4-play" },
      { label: "Kitchen", className: "gold l4-kitchen" },
      { label: "Blindbox Corner", className: "pink l4-blindbox" },
      { label: "Wash / WC", className: "service l4-toilets" },
    ],
    "5f": [
      { label: "Live Band Stage", className: "violet l5-stage" },
      { label: "Audience Seating", className: "cyan l5-audience" },
      { label: "Dance Floor", className: "gold l5-dance" },
      { label: "Sea View Bar", className: "green l5-seaview" },
    ],
  };
  const activeSplitPlans = splitFloorPlans[planKey];
  const activeSingleSplitPlan =
    typeof activeFloor?.splitPlanIndex === "number" ? activeSplitPlans?.[activeFloor.splitPlanIndex] : undefined;
  ...
  return (
    ...
    >
      ...
      <div className="rwg-yash-scene-glow" aria-hidden="true" />
      <div className="rwg-yash-scroll-rail" aria-hidden="true">
        {floors.map((floor, index) => (
          <b
            key={`rail-${floor.level}`}
            className={index === activeIndex ? "is-active" : ""}
            style={{ "--rail-tone": floor.tone } as React.CSSProperties}
          />
        ))}
      </div>
      ...
      {activeSingleSplitPlan ? (
        <div className="rwg-single-plan-stage">
          {renderPlanBody(activeSingleSplitPlan.rooms, activeSingleSplitPlan.title, `split-${activeFloor.splitPlanIndex}`, true)}
        </div>
      ) : activeSplitPlans ? (
...
function FloorScrollStory({ floors }: { floors: FloorView[] }) {
  ...
  const storyFloors = useMemo<FloorView[]>(() => {
    const createSplit = (
      floor: FloorView,
      level: string,
      title: string,
    ...
    return floors.flatMap((floor) => {
      if (floor.level === "1F") {
        return [
          createSplit(floor, "1FA", "Kids Fun House", "1 unit on the left side of 1F", "A separate family-friendly game house for children, daytime tourist traffic, prizes, and play.", 0),
          createSplit(floor, "1FB", "KTV Lounge", "2 units on 1F for KTV lounge and competition", "Two KTV lounge units for open singing, group seating, and singing competition energy.", 1),
        ];
      }
      if (floor.level === "2F") {
        return [
          createSplit(floor, "2FA", "Beauty Salon", "1 separate beauty salon unit on 2F", "Beauty services, waiting lounge, wash area, and repeat appointment traffic.", 0),
          createSplit(floor, "2FB", "Private KTV Rooms", "2 units on 2F with 4 private KTV rooms", "Four private KTV rooms for birthdays, friend groups, customer hosting, and bookings.", 1),
        ];
      }
      if (floor.level === "3F") {
        return [
          createSplit(floor, "3FA", "Beauty Salon", "1 separate beauty salon unit on 3F", "Beauty service, makeup prep, and guest pampering beside the VIP experience.", 0),
          createSplit(floor, "3FB", "VIP KTV Rooms", "2 units on 3F with 2 VIP KTV rooms", "Two premium VIP private KTV rooms for higher-spend groups and special hosting.", 1),
        ];
      }
      return [floor];
    });
  ...
  useEffect(() => {
    ...
    const sections = Array.from(story.querySelectorAll<HTMLElement>(".rwg-floor-level"));
    const observer = new IntersectionObserver(
      (entries) => {
        const centered = entries
          .filter((entry) => entry.isIntersecting)
          .sort((a, b) => b.intersectionRatio - a.intersectionRatio)[0];
        const nextIndex = centered?.target.getAttribute("data-floor-index");
        if (nextIndex) setActiveIndex(Number(nextIndex));
      },
  ...
  return (
    <div className="rwg-floor-story" ref={storyRef}>
      {storyFloors.map((floor, index) => {
        const Icon = floor.icon;
        return (
          <section
            className={`rwg-floor-level ${activeIndex === index ? "is-active" : ""}`}
          ...
          >
            <div className="rwg-floor-visual" aria-label={`${floor.level} renovated 3D floor plan`}>
              <RenovatedFloorPlanShowcase floors={storyFloors} activeIndex={index} />
              <div className="rwg-floor-tower">
                {storyFloors
                  .slice()
                  .reverse()
                  .map((towerFloor, towerIndex) => {
                    const originalIndex = storyFloors.length - 1 - towerIndex;
                    return (
                      <div
                        className={`rwg-floor-slab ${index === originalIndex ? "is-active" : ""}`}
                        key={towerFloor.level}
                        style={
                          {
                            "--floor-tone": towerFloor.tone,
                            "--floor-index": towerIndex,
                          } as React.CSSProperties
                        }
                      >
                        <span>{towerFloor.level}</span>
                      </div>
                    );
                  })}
              </div>
            ...
            <article className="rwg-floor-step">
      ...
      <div className="rwg-floor-rail" aria-hidden="true">
        {storyFloors.map((floor, index) => {
          const Icon = floor.icon;
          return (
            <a
  ...
  return (
    <main className="rwg-eco-page">
      <style>{`
        ...
        .rwg-demo-section{padding-top:36px}
        .rwg-demo-video-frame{position:relative;border-radius:28px;overflow:hidden;border:1px solid rgba(255,255,255,.14);background:linear-gradient(145deg,rgba(255,255,255,.08),rgba(255,255,255,.025));box-shadow:0 28px 90px rgba(0,0,0,.34)}
        .rwg-demo-video-frame:before{content:"";position:absolute;inset:0;border-radius:28px;box-shadow:inset 0 1px 0 rgba(255,255,255,.18);pointer-events:none;z-index:2}
        .rwg-demo-video-frame video{display:block;width:100%;max-height:78vh;aspect-ratio:16/9;object-fit:cover;background:#020617}
        .rwg-floor-story{position:relative;margin-left:calc(50% - 50vw);margin-right:calc(50% - 50vw);background:linear-gradient(180deg,rgba(3,21,26,.28),rgba(2,8,23,.82));scroll-snap-type:y proximity}
        .rwg-floor-level{position:relative;min-height:112vh;display:grid;grid-template-columns:minmax(0,1fr) minmax(300px,440px);gap:clamp(16px,3vw,42px);align-items:center;padding:clamp(28px,5vw,72px);overflow:hidden;scroll-snap-align:start}
        .rwg-floor-level:before{content:"";position:absolute;inset:0;background:radial-gradient(circle at 30% 22%,color-mix(in srgb,var(--tone),transparent 78%),transparent 34%),linear-gradient(180deg,rgba(255,255,255,.035),transparent 42%);opacity:.48;pointer-events:none}
        .rwg-floor-level:nth-child(even){grid-template-columns:minmax(300px,440px) minmax(0,1fr)}
        .rwg-floor-level:nth-child(even) .rwg-floor-visual{order:2}
        .rwg-floor-level:nth-child(even) .rwg-floor-step{order:1}
        ...
        .rwg-renovated-plan{position:relative;z-index:2;min-height:calc(100vh - clamp(36px,8vw,108px));border-radius:28px;overflow:hidden;background:radial-gradient(circle at 34% 24%,color-mix(in srgb,var(--tone),transparent 74%),transparent 34%),linear-gradient(145deg,rgba(3,21,26,.82),rgba(10,54,64,.42));border:1px solid color-mix(in srgb,var(--tone),rgba(255,255,255,.16) 35%);box-shadow:inset 0 1px 0 rgba(255,255,255,.14),0 26px 70px rgba(0,0,0,.32);display:grid;grid-template-rows:auto minmax(0,1fr) auto;padding:clamp(18px,3vw,34px);transition:border-color .35s ease,background .35s ease}
        ...
        .rwg-yash-scene-glow{position:absolute;inset:auto 12% 10%;height:36%;border-radius:999px;background:radial-gradient(circle,color-mix(in srgb,var(--tone),transparent 28%),transparent 66%);filter:blur(28px);opacity:.7;animation:rwg-yash-glow 3.6s ease-in-out infinite;pointer-events:none}
        .rwg-yash-scroll-rail{position:absolute;left:18px;top:88px;bottom:58px;z-index:5;display:grid;align-content:center;gap:10px}
        .rwg-yash-scroll-rail:before{content:"";position:absolute;left:7px;top:10px;bottom:10px;width:2px;background:linear-gradient(to bottom,transparent,rgba(255,255,255,.18),transparent)}
        .rwg-yash-scroll-rail b{position:relative;z-index:1;width:16px;height:16px;border-radius:50%;background:rgba(255,255,255,.16);border:1px solid rgba(255,255,255,.22);box-shadow:0 0 0 0 color-mix(in srgb,var(--rail-tone),transparent 80%);transition:background .35s ease,transform .35s ease,box-shadow .35s ease}
        .rwg-yash-scroll-rail b.is-active{background:var(--rail-tone);transform:scale(1.35);box-shadow:0 0 0 8px color-mix(in srgb,var(--rail-tone),transparent 82%),0 0 24px color-mix(in srgb,var(--rail-tone),transparent 40%)}
        ...
        .rwg-single-plan-stage{position:relative;z-index:1;display:grid;place-items:center;margin:18px auto;width:min(880px,84vw);animation:rwg-plan-swap .72s cubic-bezier(.2,.8,.2,1) both}
        ...
        .rwg-single-plan-stage .rwg-renovated-plan-body.is-sub-plan{width:min(720px,78vw);min-height:min(50vh,500px);transform:perspective(880px) rotateX(55deg) rotateZ(-24deg) translateY(10px)}
        ...
        .rwg-renovated-plan-body.scene-live{animation:rwg-plan-enter-live .65s cubic-bezier(.2,.8,.2,1),rwg-plan-breathe-live 7s ease-in-out infinite .65s}
        .rwg-yash-camera-ring{position:absolute;z-index:0;left:50%;top:50%;border:1px solid color-mix(in srgb,var(--tone),transparent 48%);border-radius:50%;transform:translate(-50%,-50%) translateZ(2px);pointer-events:none;opacity:.55}
        .rwg-yash-camera-ring.ring-one{width:120%;height:76%;animation:rwg-yash-ring 11s linear infinite}.rwg-yash-camera-ring.ring-two{width:86%;height:54%;animation:rwg-yash-ring 8s linear infinite reverse;opacity:.35}
        .rwg-renovated-plan-body:before,.rwg-renovated-plan-body:after,.rwg-plan-outline-cut{content:"";position:absolute;z-index:2;background:rgba(199,249,210,.72);box-shadow:0 0 18px rgba(74,222,128,.26);transform:translateZ(17px);border-radius:4px}
        .rwg-renovated-plan-body:before{left:7%;right:7%;top:31%;height:3px}
        .rwg-renovated-plan-body:after{top:8%;bottom:9%;left:52%;width:3px}
        .rwg-plan-outline-cut.cut-a{left:28%;top:31%;width:3px;height:46%}
        .rwg-plan-outline-cut.cut-b{left:28%;bottom:23%;width:26%;height:3px}
        .rwg-renovated-plan-body.plan-5f:before{left:32%;right:8%;top:68%}
        .rwg-renovated-plan-body.plan-5f:after{left:36%;top:43%;bottom:0}
        .rwg-renovated-plan-body.plan-5f .cut-a{left:36%;top:43%;height:3px;width:34%}
        .rwg-renovated-plan-body.plan-5f .cut-b{left:70%;top:43%;width:3px;height:25%}
        .rwg-renovated-plan-body.plan-1f:before{left:8%;right:8%;top:22%}.rwg-renovated-plan-body.plan-1f:after{left:43%;top:8%;bottom:22%}.rwg-renovated-plan-body.plan-1f .cut-a{left:8%;top:66%;width:44%;height:3px}.rwg-renovated-plan-body.plan-1f .cut-b{left:72%;top:8%;width:3px;height:28%}
        .rwg-renovated-plan-body.plan-2f:before,.rwg-renovated-plan-body.plan-3f:before{left:8%;right:8%;top:31%}.rwg-renovated-plan-body.plan-2f:after,.rwg-renovated-plan-body.plan-3f:after{left:35%;top:8%;bottom:8%}.rwg-renovated-plan-body.plan-2f .cut-a,.rwg-renovated-plan-body.plan-3f .cut-a{left:58%;top:8%;width:3px;height:28%}.rwg-renovated-plan-body.plan-2f .cut-b,.rwg-renovated-plan-body.plan-3f .cut-b{left:35%;top:58%;width:46%;height:3px}
        .rwg-renovated-plan-body.plan-4f:before{left:8%;right:8%;top:34%}.rwg-renovated-plan-body.plan-4f:after{left:57%;top:8%;bottom:20%}.rwg-renovated-plan-body.plan-4f .cut-a{left:72%;top:8%;width:3px;height:28%}.rwg-renovated-plan-body.plan-4f .cut-b{left:8%;top:66%;width:41%;height:3px}
        ...
        .rwg-plan-room.cyan{background:linear-gradient(135deg,#22d3ee,#0e7490);color:#031316}.rwg-plan-room.gold{background:linear-gradient(135deg,#f8d477,#b7791f);color:#1b1300}.rwg-plan-room.pink{background:linear-gradient(135deg,#f472b6,#be185d);color:#190310}.rwg-plan-room.green{background:linear-gradient(135deg,#34d399,#047857);color:#03170e}.rwg-plan-room.violet{background:linear-gradient(135deg,#a78bfa,#6d28d9);color:#100522}.rwg-plan-room.service{background:linear-gradient(135deg,#e5e7eb,#94a3b8);color:#0f172a}
        .rwg-plan-room.l1-main-left{left:9%;top:8%;width:32%;height:54%}.rwg-plan-room.l1-main-right{left:46%;top:8%;width:24%;height:54%}.rwg-plan-room.l1-front-left{left:9%;top:69%;width:27%;height:23%}.rwg-plan-room.l1-front-right{left:40%;top:69%;width:25%;height:23%}.rwg-plan-room.l1-toilets{right:7%;top:9%;width:18%;height:16%}.rwg-plan-room.l1-store{right:7%;top:28%;width:18%;height:20%}
        .rwg-plan-room.l2-room-a{left:9%;top:8%;width:23%;height:23%}.rwg-plan-room.l2-room-b{left:36%;top:8%;width:19%;height:23%}.rwg-plan-room.l2-room-c{left:61%;top:8%;width:18%;height:23%}.rwg-plan-room.l2-room-d{left:62%;top:38%;width:18%;height:22%}.rwg-plan-room.l2-salon{left:9%;bottom:8%;width:42%;height:31%}.rwg-plan-room.l2-toilets{right:7%;top:8%;width:15%;height:21%}
        ...
        .rwg-plan-core.lift{width:12%;height:12%;left:44%;top:45%}.rwg-plan-core.stair{width:15%;height:28%;right:7%;top:48%;background:repeating-linear-gradient(0deg,rgba(248,212,119,.92) 0 3px,rgba(3,21,26,.88) 3px 9px)}.rwg-plan-core.stair-b{left:8%;right:auto;top:66%;height:22%}
        .plan-5f .rwg-plan-core.lift{display:none}.plan-5f .rwg-plan-core.stair-a{left:43%;right:auto;top:55%;width:17%;height:29%}.plan-5f .rwg-plan-core.stair-b{display:none}
        .plan-1f .rwg-plan-core.lift{left:45%;top:43%}.plan-1f .rwg-plan-core.stair-a{right:7%;top:50%}.plan-1f .rwg-plan-core.stair-b{left:8%;top:65%}
        .plan-2f .rwg-plan-core.lift,.plan-3f .rwg-plan-core.lift,.plan-4f .rwg-plan-core.lift{left:43%;top:45%}.plan-2f .rwg-plan-core.stair-a,.plan-3f .rwg-plan-core.stair-a,.plan-4f .rwg-plan-core.stair-a{right:7%;top:12%}.plan-2f .rwg-plan-core.stair-b,.plan-3f .rwg-plan-core.stair-b,.plan-4f .rwg-plan-core.stair-b{left:25%;top:68%}
        .rwg-floor-tower{position:absolute;right:clamp(22px,5vw,76px);bottom:clamp(22px,5vw,72px);z-index:3;width:min(300px,32vw);justify-self:center;transform-style:preserve-3d;transform:rotateX(58deg) rotateZ(-30deg);animation:rwg-floor-float 8s ease-in-out infinite}
        ...
        .rwg-floor-step{min-height:420px;border-radius:24px;padding:28px;display:grid;grid-template-columns:78px minmax(0,1fr);gap:22px;align-items:center;background:linear-gradient(135deg,rgba(3,21,26,.82),rgba(255,255,255,.055));border:1px solid rgba(255,255,255,.11);box-shadow:0 24px 80px rgba(0,0,0,.24);position:relative;overflow:hidden;z-index:3;transform:translateY(70px);opacity:.25;transition:border-color .45s ease,box-shadow .45s ease,transform .7s cubic-bezier(.2,.8,.2,1),opacity .7s ease}
        .rwg-floor-level.is-active .rwg-floor-step{border-color:color-mix(in srgb,var(--tone),white 18%);box-shadow:0 28px 90px color-mix(in srgb,var(--tone),transparent 78%),0 24px 80px rgba(0,0,0,.24);transform:translateY(0);opacity:1}
        ...
        .rwg-floor-unit-split{list-style:none;margin:18px 0 0;padding:0;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;max-width:680px}
        .rwg-floor-unit-split li{position:relative;min-height:82px;display:flex;align-items:flex-start;padding:16px 14px 14px 42px;border-radius:15px;background:linear-gradient(135deg,rgba(255,255,255,.09),rgba(255,255,255,.045));border:1px solid rgba(255,255,255,.12);color:rgba(255,255,255,.82);font-size:13px;font-weight:750;line-height:1.45;box-shadow:inset 0 1px 0 rgba(255,255,255,.09)}
        .rwg-floor-unit-split li:before{content:"";position:absolute;left:15px;top:18px;width:12px;height:12px;border-radius:50%;background:var(--tone);box-shadow:0 0 0 5px color-mix(in srgb,var(--tone),transparent 84%),0 0 18px color-mix(in srgb,var(--tone),transparent 45%)}
        .rwg-floor-unit-split li::first-line{color:#fff;font-weight:950}
        ...
        .rwg-floor-rail a span{display:none}
        .rwg-floor-rail a.is-active{background:var(--tone);color:#061316;transform:translateY(-4px);box-shadow:0 14px 30px color-mix(in srgb,var(--tone),transparent 62%)}
        ...
        @keyframes rwg-plan-swap{0%{opacity:.2;transform:translateY(60px) scale(.92) rotate(-1deg);filter:blur(8px)}100%{opacity:1;transform:translateY(0) scale(1) rotate(0);filter:blur(0)}}
        @keyframes rwg-active-copy-in{0%{opacity:.3;transform:translateY(18px)}100%{opacity:1;transform:translateY(0)}}
        ...
        @keyframes rwg-plan-enter-private{0%{opacity:.15;transform:perspective(900px) rotateX(70deg) rotateZ(-18deg) translateY(84px) scale(.84)}100%{opacity:1;transform:perspective(900px) rotateX(58deg) rotateZ(-22deg) translateY(16px) scale(1)}}
        @keyframes rwg-plan-enter-vip{0%{opacity:.15;transform:perspective(900px) rotateX(64deg) rotateZ(-52deg) translateY(84px) scale(.84)}100%{opacity:1;transform:perspective(900px) rotateX(54deg) rotateZ(-42deg) translateY(18px) scale(1)}}
        @keyframes rwg-plan-enter-pet{0%{opacity:.15;transform:perspective(900px) rotateX(70deg) rotateZ(-32deg) translateY(84px) scale(.84)}100%{opacity:1;transform:perspective(900px) rotateX(58deg) rotateZ(-30deg) translateY(16px) scale(1)}}
        @keyframes rwg-plan-enter-live{0%{opacity:.15;transform:perspective(900px) rotateX(62deg) rotateZ(-8deg) translateY(84px) scale(.84)}100%{opacity:1;transform:perspective(900px) rotateX(52deg) rotateZ(-14deg) translateY(18px) scale(1)}}
        ...
        @keyframes rwg-plan-breathe-private{0%,100%{transform:perspective(900px) rotateX(58deg) rotateZ(-22deg) translateY(16px)}50%{transform:perspective(900px) rotateX(60deg) rotateZ(-18deg) translateY(5px)}}
        @keyframes rwg-plan-breathe-vip{0%,100%{transform:perspective(900px) rotateX(54deg) rotateZ(-42deg) translateY(18px)}50%{transform:perspective(900px) rotateX(56deg) rotateZ(-38deg) translateY(6px)}}
        @keyframes rwg-plan-breathe-pet{0%,100%{transform:perspective(900px) rotateX(58deg) rotateZ(-30deg) translateY(16px)}50%{transform:perspective(900px) rotateX(60deg) rotateZ(-26deg) translateY(5px)}}
        @keyframes rwg-plan-breathe-live{0%,100%{transform:perspective(900px) rotateX(52deg) rotateZ(-14deg) translateY(18px)}50%{transform:perspective(900px) rotateX(54deg) rotateZ(-10deg) translateY(6px)}}
