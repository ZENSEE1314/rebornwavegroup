...
export default function StaffFeedback() {
  ...
  async function submit(){if(!companyId||!messageText.trim()||(category==="staff"&&!staffId))return;setBusy(true);setMessage("");try{await call("/api/v1/company/feedback",companyId,{method:"POST",body:JSON.stringify({category,staffUserId:category==="staff"?staffId:null,rating,subject,message:messageText})});setMessage("Thank you. Your feedback was sent to the company team.");setMessageText("");setSubject("");setStaffId("");setRating(5)}catch(e:any){setMessage(e.message)}finally{setBusy(false)}}
  return <RebornLayout active="staff-feedback" title="FEEDBACK"><div className="rounded-3xl border border-white/10 bg-white/5 p-5"><div className="mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-amber-400/15"><MessageSquareMore className="h-7 w-7 text-amber-300"/></div><h1 className="text-2xl font-extrabold">Help us improve</h1><p className="mb-5 mt-1 text-sm text-white/50">Review our service or a staff member, or suggest an improvement. Staff ratings also appear on the weekly leaderboard.</p>
    <div className="space-y-3"><select className={box} value={companyId||""} onChange={e=>setCompanyId(Number(e.target.value))}>{companies.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><div className="grid grid-cols-3 gap-2">{[["service","Our service"],["staff","Our staff"],["improvement","Improvement"]].map(([v,l])=><button key={v} onClick={()=>setCategory(v)} className={`rounded-xl border p-3 text-xs font-bold ${category===v?"border-amber-300 bg-amber-300/15 text-amber-200":"border-white/10 text-white/60"}`}>{l}</button>)}</div>
      {category==="staff"&&<select className={box} value={staffId} onChange={e=>setStaffId(e.target.value)}><option value="">Select staff member</option>{staff.map(s=><option key={s.user_id} value={s.user_id}>{[s.first_name,s.last_name].filter(Boolean).join(" ")||s.email} · {s.position_name||s.role}</option>)}</select>}
      {category!=="improvement"&&<div className="flex justify-center gap-2 py-3">{[1,2,3,4,5].map(n=><button key={n} aria-label={`${n} stars`} onClick={()=>setRating(n)}><Star className={`h-9 w-9 ${n<=rating?"fill-amber-300 text-amber-300":"text-white/20"}`}/></button>)}</div>}
      <input className={box} placeholder="Subject (optional)" value={subject} onChange={e=>setSubject(e.target.value)}/><textarea className={box+" min-h-28"} placeholder="Tell us what was good or what we should improve" value={messageText} onChange={e=>setMessageText(e.target.value)}/><button disabled={busy||!messageText.trim()||(category==="staff"&&!staffId)} onClick={submit} className="w-full rounded-xl bg-gradient-to-r from-amber-500 to-yellow-300 py-3 font-bold text-black disabled:opacity-40">{busy?"Sending…":"Send feedback"}</button>{message&&<p className="rounded-xl bg-white/5 p-3 text-sm text-amber-100">{message}</p>}</div>
  </div></RebornLayout>;
}
